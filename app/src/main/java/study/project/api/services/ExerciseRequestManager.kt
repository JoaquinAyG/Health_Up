...
class ExerciseRequestManager : ApiRequestManager<ExerciseResponse> {
    ...
    override fun getId(id: Int): ExerciseResponse.Results {
        return getUrlObject(
            URL("$API_EXERCISE_URL$id/$FORMAT_JSON"),
            ExerciseResponse.Results::class.java
        )
    }
    ...
    override fun getAll(): MutableCollection<ExerciseResponse> {
        ...
        var exerciseResponse =
            ExerciseResponse(null, API_EXERCISE_URL + FORMAT_JSON, null, arrayListOf())
        do {
            exerciseResponse =
                getUrlObject(URL(exerciseResponse.next), ExerciseResponse::class.java)
