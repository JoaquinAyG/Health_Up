...
object HealthApiManagerClient {
    ...
    fun getAllExercises(): List<ExerciseView> {
        ...
        exerciseRequestManager.getAll().forEach { it.results.forEach { exerciseList.add(ExerciseView().fromResult(it)) } }
        return exerciseList
    }

    fun getAllExerciseImages(): List<ExerciseImageView> {
        val exerciseImageList = mutableListOf<ExerciseImageView>()
        exerciseImageRequestManager.getAll().forEach { it.results.forEach { exerciseImageList.add(
    ...
    fun getAllExerciseComments(): List<ExerciseCommentView> {
        ...
        exerciseCommentRequestManager.getAll().forEach { it.results.forEach { exerciseCommentList.add(
    ...
    fun getAllMuscles(): List<MuscleView> {
        ...
        muscleRequestManager.getAll().forEach { it.results.forEach { muscleList.add(MuscleView().fromResult(it)) } }
        return muscleList
    }

    fun getAllExerciseInfo(): List<ExerciseInfoView>{
        val exerciseInfoList = mutableListOf<ExerciseInfoView>()
        exerciseInfoRequestManager.getAll().forEach { it.results.forEach { exerciseInfoList.add(
    ...
    fun getAllExerciseCategories(): List<ExerciseCategoryView>{
        ...
        exerciseCategoryRequestManager.getAll().forEach { it.results.forEach { exerciseCategoryList.add(
